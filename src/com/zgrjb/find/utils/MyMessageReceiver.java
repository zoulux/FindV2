...
public class MyMessageReceiver extends BroadcastReceiver {
							...
							if (currentUser != null) {// 有登陆用户
								if (toId.equals(currentUser.getObjectId())) {
									if (ehList.size() > 0) {// 有监听的时候，传递下去
										...
										// 这里需要写添加好友 // showOtherNotify(context,
	...
	public void showMsgNotify(Context context, BmobMsg msg) {
		...
		boolean isAllowVoice = CustomApplcation.getInstance()
				.getSharePreferenceUtil().isAllowVoice();
		boolean isAllowVibrate = CustomApplcation.getInstance()
				.getSharePreferenceUtil().isAllowVibrate();
	...
	public void showOtherNotify(Context context, String username, String toId,
		...
		boolean isAllow = CustomApplcation.getInstance()
				.getSharePreferenceUtil().isAllowPushNotify();
		boolean isAllowVoice = CustomApplcation.getInstance()
				.getSharePreferenceUtil().isAllowVoice();
		boolean isAllowVibrate = CustomApplcation.getInstance()
				.getSharePreferenceUtil().isAllowVibrate();

...
public class MyMessageReceiver extends BroadcastReceiver {
					...
					if(TextUtils.isEmpty(tag)){//不携带tag标签--此可接收陌生人的消息
						...
						BmobChatManager.getInstance(context).createReceiveMsg(json, new OnReceiveListener() {
							...
							public void onSuccess(BmobMsg msg) {
								...
								boolean isAllow = CustomApplcation.getInstance().getSharePreferenceUtil().isAllowPushNotify(); //允许弹出Notify
	...
	//	这里需要写添加好友			//					showOtherNotify(context, message.getFromname(), toId,  message.getFromname()+"请求添加好友", NewFriendActivity.class);
									}
								}
							}
						...
						//添加并同意
							...
							//显示通知
							showOtherNotify(context, username, toId,  username+"同意添加您为好友", MainUIActivity.class);
	...
	public void showMsgNotify(Context context,BmobMsg msg) {
		...
		boolean isAllowVoice = CustomApplcation.getInstance().getSharePreferenceUtil().isAllowVoice();
		boolean isAllowVibrate = CustomApplcation.getInstance().getSharePreferenceUtil().isAllowVibrate();
	...
	public void showOtherNotify(Context context,String username,String toId,String ticker,Class<?> cls){
		boolean isAllow = CustomApplcation.getInstance().getSharePreferenceUtil().isAllowPushNotify();
		boolean isAllowVoice = CustomApplcation.getInstance().getSharePreferenceUtil().isAllowVoice();
		boolean isAllowVibrate = CustomApplcation.getInstance().getSharePreferenceUtil().isAllowVibrate();
		if(isAllow && currentUser!=null && currentUser.getObjectId().equals(toId)){
			//同时提醒通知
//需要写添加新朋友	//		BmobNotifyManager.getInstance(context).showNotify(isAllowVoice,isAllowVibrate,R.drawable.ic_launcher, ticker,username, ticker.toString(),NewFriendActivity.class);
		}
	}
